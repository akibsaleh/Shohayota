...
const ApplicationForm = () => {
  ...
  const formSubmit = async (data) => {
    ...
    try {
      ...
      const imageRef = ref(
        storage,
        `images/${data.name}/${singleImage.name + dateTime}`
      );
      ...
      for (let i = 0; i < otherFiles.length; i++) {
        const otherFileRef = ref(
          storage,
          `images/${data.name}/otherFiles/${otherFiles[i].name + dateTime}`
        );
                ...
                <div className="w-full">
                  <div className="flex items-center justify-start w-full pt-2 pb-1.5 px-3 rounded-md border-thunder-300 border bg-haze divide-x divide-solid gap-x-2.5">
                    <span className="flex gap-x-1 leading-[26px] text-thunder-500 font-archivo">
                      ...
                      <span className={isBn ? "font-nsb" : "font-archivo"}>
                        {t("contact.phoneInputPrefix")}
                      </span>
                    </span>
                    <input
                      {...register("phone", {
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-[210px_auto] gap-6 items-center">
                ...
                <div className="w-80 h-40 relative">
                  {otherFiles.length > 0 ? (
                    <div
                      className={`grid grid-cols-${otherFiles.length} gap-2 w-80 h-40 overflow-hidden border-plant-700 border rounded-md p-1.5`}
                    >
