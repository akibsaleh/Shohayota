...
const ApplicationForm = () => {
  ...
  const handleSingleFileChange = (e) => {
    ...
    if (!newFile) return;
    const checkFileTypes = ['image/jpeg', 'image/png', 'image/webp', 'application/pdf'];
    if (!checkFileTypes.includes(newFile.type)) {
      toast.error('Please select a valid image (JPEG, PNG or WEBP) or a PDF file.');
      e.target.value = '';
      return;
    }
  ...
  const handleMultipleImageChange = (e) => {
    ...
    const validFiles = newFiles.filter((file) => checkFileTypes.includes(file.type));
        ...
        <div className="w-full relative">
          <div className="flex gap-4 md:gap-10 w-full justify-center items-center pb-10">
            <div
              ...
              onClick={() => (allowNext ? setFormStep(0) : undefined)}
            >
            ...
            <div
              className="flex gap-3.5 cursor-pointer text-thunder-500 text-base md:text-[22px] font-normal"
              onClick={() => (allowNext ? setFormStep(1) : undefined)}
            >
